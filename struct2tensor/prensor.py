...
class LeafNodeTensor:
    ...
    def __init__(self, parent_index: tf.Tensor, values: tf.Tensor, is_repeated: bool):
    ...
    def __str__(self):
        return "{} {}".format(
            "repeated" if self.is_repeated else "optional", str(self.values.dtype)
        )


def create_required_leaf_node(values: tf.Tensor) -> LeafNodeTensor:
    """Create a required leaf node."""
    return LeafNodeTensor(tf.range(tf.size(values, out_type=tf.int64)), values, False)
...
class _PrensorTypeSpec(tf.TypeSpec):
    ...
    __slots__ = ["_is_repeated", "_node_type", "_value_dtype", "_children_specs"]
    ...
    def _append_to_components(self, value: "Prensor", components: List[tf.Tensor]):
        node = value.node
    ...
    def _from_component_iter(self, component_iter: Iterator[tf.Tensor]) -> "Prensor":
        if self._node_type == self._NodeType.ROOT:
        ...
        for step, child_spec in self._children_specs:
            step_to_child[step] = child_spec._from_component_iter(  # pylint: disable=protected-access
                component_iter
            )
    ...
    def _append_to_component_specs(self, component_specs: List[tf.TensorSpec]):
        if self._node_type == self._NodeType.ROOT:
            ...
            component_specs.append(tf.TensorSpec([None], tf.int64))
            component_specs.append(tf.TensorSpec([None], self._value_dtype))
    ...
    def _serialize(self) -> Tuple[bool, int, tf.DType, Tuple]:  # pylint: disable=g-bare-generic
        return (
            ...
            self._value_dtype,
            tuple(
                (step, child_spec._serialize())  # pylint: disable=protected-access
                for step, child_spec in self._children_specs
            ),
        )
    ...
    def _deserialize(cls, serialization: Tuple[bool, int, tf.DType, Tuple]):  # pylint: disable=g-bare-generic
        ...
        children_specs = [
            (step, cls._deserialize(child_serialization))
...
class Prensor(composite_tensor.CompositeTensor):
    ...
    def __init__(
        self, node: NodeTensor, children: "collections.OrderedDict[path.Step, Prensor]"
    ):
        """Construct a Prensor.
    ...
    def _type_spec(self) -> _PrensorTypeSpec:
        ...
        return _PrensorTypeSpec(
            ...
            node_type,
            value_dtype,
            [(step, child._type_spec) for step, child in self.get_children().items()],
        )
        # pylint: enable=protected-access


def create_prensor_from_descendant_nodes(
    nodes: Mapping[path.Path, NodeTensor],
) -> "Prensor":
...
def create_prensor_from_root_and_children(
    root: NodeTensor, children: Mapping[path.Step, Prensor]
) -> Prensor:
...
class _LeafNodePath:
    ...
    def __init__(
        self,
        head: RootNodeTensor,
        middle: Sequence[ChildNodeTensor],
        tail: LeafNodeTensor,
    ):
        self._head = head
        self._middle = middle
...
def _get_leaf_node_path(p: path.Path, t: Prensor) -> _LeafNodePath:
    ...
    child_node_pairs = [
        (t.get_descendant_or_error(ancestor).node, ancestor)
        for ancestor in strict_ancestor_paths
    ]
    bad_struct_paths = [
        ancestor
        for node, ancestor in child_node_pairs
        if not isinstance(node, ChildNodeTensor)
    ]
    if bad_struct_paths:
        raise ValueError(
            "Expected ChildNodeTensor at {} in {}".format(
                " ".join([str(x) for x in bad_struct_paths]), str(t)
            )
        )
    # This should select all elements: the isinstance is for type-checking.
    child_nodes = [
        node for node, ancestor in child_node_pairs if isinstance(node, ChildNodeTensor)
    ]
...
def _get_node_path_parent(p: Union[_LeafNodePath, _ChildNodePath]) -> _ChildNodePath:
...
def _get_dewey_encoding(
    p: Union[_LeafNodePath, _ChildNodePath],
    ...
    current_size = tf.maximum(tf.reshape(tf.reduce_max(positional_index) + 1, [1]), [0])
    if not p.middle:
        if p.tail.is_repeated:
            return (
                tf.stack([p.tail.parent_index, positional_index], axis=1),
                tf.concat([parent_size, current_size], 0),
            )
        return tf.expand_dims(p.tail.parent_index, -1), parent_size
    parent_dewey_encoding, parent_size = _get_dewey_encoding(_get_node_path_parent(p))
    if p.tail.is_repeated:
        positional_index_as_matrix = tf.expand_dims(p.tail.get_positional_index(), -1)
        indices = tf.concat(
            [
...
def from_value_rowids_bridge(values, value_rowids=None, nrows=None, validate=True):
