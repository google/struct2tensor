...
def project(
    expr: expression.Expression, paths: Sequence[path.Path]
    ...
    if missing_paths:
        raise ValueError(
            "{} Path(s) missing in project: {}".format(
...
) -> Mapping[path.Step, List[path.Path]]:
...
class _ProjectExpression(expression.Expression):
    ...
    def _get_child_impl(self, field_name: path.Step) -> Optional[expression.Expression]:
        ...
        if original is None:
            raise ValueError(f"Project a field that doesn't exist: {field_name}")
