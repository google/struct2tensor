...
class PromoteTest(absltest.TestCase):
    ...
    def test_promote_anonymous(self):
        ...
        self.assertIs(
            expr.get_descendant_or_error(path.Path(["user", "friends"])), sources[0]
        )
    ...
    def test_promote_with_schema(self):
        ...
        ).apply_schema(prensor_test_util.create_big_prensor_schema())
    ...
    def test_promote_with_schema_dense_parent(self):
        ...
        self.assertEqual(
            new_schema_feature.lifecycle_stage, schema_pb2.LifecycleStage.ALPHA
        )
        self.assertEqual(
    ...
    def test_lifecycle_stage(self):
        ...
        self.assertEqual(
            schema_pb2.LifecycleStage.DEPRECATED,
            _check_lifecycle_stage(
                schema_pb2.LifecycleStage.DISABLED, schema_pb2.LifecycleStage.DEPRECATED
            ),
        )
        ...
        self.assertEqual(
            schema_pb2.LifecycleStage.DISABLED,
            _check_lifecycle_stage(
                schema_pb2.LifecycleStage.PLANNED, schema_pb2.LifecycleStage.DISABLED
            ),
        )
        ...
        self.assertEqual(
            schema_pb2.LifecycleStage.DEPRECATED,
            _check_lifecycle_stage(
                schema_pb2.LifecycleStage.PLANNED, schema_pb2.LifecycleStage.DEPRECATED
            ),
        )
        ...
        self.assertEqual(
            schema_pb2.LifecycleStage.PLANNED,
            _check_lifecycle_stage(
                schema_pb2.LifecycleStage.ALPHA, schema_pb2.LifecycleStage.PLANNED
            ),
        )
        ...
        self.assertEqual(
            schema_pb2.LifecycleStage.ALPHA,
            _check_lifecycle_stage(
                schema_pb2.LifecycleStage.DEBUG_ONLY, schema_pb2.LifecycleStage.ALPHA
            ),
        )
        self.assertEqual(
            schema_pb2.LifecycleStage.ALPHA,
        ...
        self.assertEqual(
            schema_pb2.LifecycleStage.UNKNOWN_STAGE,
            _check_lifecycle_stage(
                schema_pb2.LifecycleStage.BETA, schema_pb2.LifecycleStage.UNKNOWN_STAGE
            ),
        )
        self.assertEqual(
            schema_pb2.LifecycleStage.UNKNOWN_STAGE,
        ...
        self.assertEqual(
            schema_pb2.LifecycleStage.BETA,
            _check_lifecycle_stage(
                schema_pb2.LifecycleStage.PRODUCTION, schema_pb2.LifecycleStage.BETA
            ),
        )
    ...
    def test_promote(self):
        ...
        new_root = promote.promote(expr, path.Path(["user", "friends"]), "new_field")
    ...
    def test_promote_substructure(self):
        ...
        self.assertIsNotNone(new_field)
        self.assertTrue(new_field.is_repeated)
        self.assertEqual(new_field.known_field_names(), frozenset(["bar", "keep_me"]))
    ...
    def test_promote_substructure_then_leaf(self):
        ...
        new_root = expr.promote(path.Path(["event", "doc"]), "new_field").promote(
            path.Path(["new_field", "bar"]), "new_bar"
        )
        ...
        new_field_bar_node = expression_test_util.calculate_value_slowly(new_field_bar)
    ...
    def test_promote_leaf_then_substructure(self):
        ...
        new_root = expr.promote(
            path.Path(["event", "doc", "nested_child", "bar"]), "new_bar"
        ...
        self.assertEqual(
            new_doc.known_field_names(), frozenset(["nested_child", "new_bar"])
        )
        ...
        self.assertEqual(
            nested_child_expr.known_field_names(), frozenset(["bar", "keep_me"])
        )
...
class PromoteValuesTest(tf.test.TestCase):
    ...
    def test_promote_and_calculate_substructure(self):
        ...
        keep_me_expr = new_root.get_descendant_or_error(new_path.get_child("keep_me"))
    ...
    def test_promote_substructure_with_schema(self):
        expr = create_expression.create_expression_from_prensor(
            prensor_test_util.create_deep_prensor()
        ).apply_schema(prensor_test_util.create_deep_prensor_schema())
        ...
        self.assertProtoEquals(
            new_schema_feature.struct_domain, original_schema.struct_domain
        )

        bar_schema = new_root.get_descendant_or_error(
            new_field_path.concat(path.Path(["bar"]))
        ).schema_feature
        ...
        keep_me_schema = new_root.get_descendant_or_error(
            new_field_path.concat(path.Path(["keep_me"]))
        ).schema_feature
