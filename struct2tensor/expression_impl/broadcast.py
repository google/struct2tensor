...
class _BroadcastExpression(expression.Leaf):
    ...
    def calculate(
        ...
        return prensor.LeafNodeTensor(
            broadcasted_to_sibling_index, new_values, self.is_repeated
        )
...
class _RecalculateExpression(expression.Expression):
  ...
  """
    ...
    def calculate(
        ...
        [broadcasted_to_sibling_index, index_to_values] = (
            struct2tensor_ops.equi_join_any_indices(
        ...
        return prensor.ChildNodeTensor(
            broadcasted_to_sibling_index, self.is_repeated, index_to_values
        )
...
def _broadcast_impl(
    ...
    broadcast_expression_factory = (
        _BroadcastExpression if origin_expression.is_leaf else _BroadcastChildExpression
    )
