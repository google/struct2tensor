...
class SliceExpressionTest(tf.test.TestCase):
  ...
  def test_slice_end(self):
    with self.test_session(use_gpu=False) as sess:
      ...
      self.assertAllEqual(
          result.get_descendant_or_error(path.Path(
              ["new_doc", "keep_me"])).node.parent_index, [0, 1])
      self.assertAllEqual(
          result.get_descendant_or_error(path.Path(
              ["new_doc", "keep_me"])).node.values, [False, True])
      self.assertAllEqual(
          result.get_descendant_or_error(path.Path(
              ["new_doc", "bar"])).node.parent_index, [0, 1, 1])
      self.assertAllEqual(
          result.get_descendant_or_error(path.Path(
              ["new_doc", "bar"])).node.values, [b"a", b"b", b"c"])
  ...
  def test_slice_begin(self):
    ...
    with self.test_session(use_gpu=False) as sess:
      ...
      self.assertAllEqual(
          result.get_descendant_or_error(path.Path(
              ["new_doc", "keep_me"])).node.parent_index, [])
      self.assertAllEqual(
          result.get_descendant_or_error(path.Path(
              ["new_doc", "keep_me"])).node.values, [])
      self.assertAllEqual(
          result.get_descendant_or_error(path.Path(
