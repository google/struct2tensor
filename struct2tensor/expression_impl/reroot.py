...
def create_proto_index_field(
    root: expression.Expression, new_field_name: path.Step
) -> expression.Expression:
...
class _RerootExpression(expression.Expression):
    ...
    def __init__(self, original_root: expression.Expression, field_name: path.Step):
        super().__init__(
        ...
        if self._new_root.type is not None:
            raise ValueError(
                f"New root must be a message type: {str(self._field_name)}"
            )
