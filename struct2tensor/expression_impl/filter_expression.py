...
def filter_by_sibling(
    ...
    sibling = expr.get_descendant_or_error(parent_path.get_child(sibling_field_name))
...
class _FilterChildNodeTensor(prensor.ChildNodeTensor):
    ...
    def __init__(
        self, parent_index: tf.Tensor, is_repeated: bool, indices_to_keep: tf.Tensor
    ):
...
def _filter_by_self_indices_to_keep(
    node_value: prensor.NodeTensor, self_indices_to_keep: tf.Tensor
) -> prensor.NodeTensor:
...
def _filter_by_parent_indices_to_keep(
    ...
    parent_indices_to_keep: tf.Tensor,
) -> prensor.NodeTensor:
    """Filter by parent indices to keep."""
    [new_parent_index, self_indices_to_keep] = struct2tensor_ops.equi_join_indices(
        parent_indices_to_keep, node_value.parent_index
    )
    if isinstance(node_value, prensor.ChildNodeTensor):
        return _FilterChildNodeTensor(
            new_parent_index, node_value.is_repeated, self_indices_to_keep
        )
    if isinstance(node_value, prensor.LeafNodeTensor):
        return prensor.LeafNodeTensor(
            new_parent_index,
...
class _FilterChildByParentIndicesToKeepExpression(expression.Expression):
    ...
    def calculate(
        ...
        if not isinstance(
            parent_value, (_FilterChildNodeTensor, _FilterRootNodeTensor)
        ):
            raise ValueError("Parent must be a filtered node")

        if not isinstance(
            origin_value, (prensor.ChildNodeTensor, prensor.LeafNodeTensor)
        ):
            raise ValueError("Original must be a child or leaf node")

        parent_indices_to_keep = parent_value.indices_to_keep
        return _filter_by_parent_indices_to_keep(origin_value, parent_indices_to_keep)
...
class _FilterBySiblingExpression(expression.Expression):
    ...
    def __init__(self, origin: expression.Expression, sibling: expression.Expression):
        super().__init__(
            origin.is_repeated,
            origin.type,
            validate_step_format=origin.validate_step_format,
...
class _FilterByChildExpression(expression.Expression):
    ...
    def __init__(self, origin: expression.Expression, child: expression.Expression):
        super().__init__(
            origin.is_repeated,
            origin.type,
            validate_step_format=origin.validate_step_format,
