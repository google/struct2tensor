...
def map_sparse_tensor(
    ...
    """
    return _map_sparse_tensor_impl(
        root, root_path, paths, operation, is_repeated, dtype, new_field_name
    )[0]


def map_ragged_tensor(
    root: expression.Expression,
    root_path: path.Path,
    paths: Sequence[path.Path],
    ...
    Returns:
      ...
      root_path.get_child(new_field_name), with the result of the operation.
    """
    return _map_ragged_tensor_impl(
        root, root_path, paths, operation, is_repeated, dtype, new_field_name
    )[0]
...
class _MapPrensorExpression(expression.Expression):
    ...
    def __init__(
        ...
        operation: Callable[
            [prensor.Prensor, calculate_options.Options], prensor.LeafNodeTensor
        ],
        is_repeated: bool,
        dtype: tf.DType,
    ):
...
def _as_leaf_node_no_checks(
    sparse_tensor: tf.SparseTensor, is_repeated: bool
) -> prensor.LeafNodeTensor:
...
def _as_leaf_node_with_checks(
    sparse_tensor: tf.SparseTensor, is_repeated: bool, required_batch_size: tf.Tensor
) -> prensor.LeafNodeTensor:
    """Take a SparseTensor and create a LeafNodeTensor, with checks."""
    assertions = [tf.assert_equal(sparse_tensor.dense_shape[0], required_batch_size)]
    if is_repeated:
...
def _as_leaf_node(
    ...
    if options.sparse_checks:
        return _as_leaf_node_with_checks(
            sparse_tensor, is_repeated, required_batch_size
        )
    return _as_leaf_node_no_checks(sparse_tensor, is_repeated)
...
def _map_sparse_tensor_impl(
    ...
    def new_op(
        pren: prensor.Prensor, options: calculate_options.Options
    ) -> prensor.LeafNodeTensor:
        ...
        result = _as_leaf_node(
            result_as_tensor, is_repeated, sparse_tensors[0].dense_shape[0], options
        )
        if result.values.dtype != dtype:
            raise ValueError(
                f"Type unmatched: actual ({str(result.values.dtype)})!= expected ({str(dtype)})"
            )
        return result

    return _map_prensor_impl(
        root, root_path, paths, new_op, is_repeated, dtype, new_field_name
    )
...
def _ragged_as_leaf_node(
    ...
    reference_size_dim = tf.compat.dimension_at_index(
        reference_ragged_tensor.shape, 0
    ).value
    ...
    if assertions:
        with tf.control_dependencies(assertions):
            ...
            return prensor.LeafNodeTensor(
                parent_index, ragged_tensor.values, is_repeated
            )
    else:
        parent_index = ragged_tensor.value_rowids()
        return prensor.LeafNodeTensor(parent_index, ragged_tensor.values, is_repeated)


def _map_ragged_tensor_impl(
    root: expression.Expression,
    root_path: path.Path,
    paths: Sequence[path.Path],
    operation: Callable[..., tf.RaggedTensor],
    ...
    def new_op(
        tree: prensor.Prensor, options: calculate_options.Options
    ) -> prensor.LeafNodeTensor:
        ...
        result = _ragged_as_leaf_node(
            result_as_tensor, is_repeated, ragged_tensors[0], options
        )
        if result.values.dtype != dtype:
            raise ValueError(
                f"Type unmatched: actual ({str(result.values.dtype)})!= expected ({str(dtype)})"
