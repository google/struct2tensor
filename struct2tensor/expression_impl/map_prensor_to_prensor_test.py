    ...
    def test_map_prensor_to_prensor(self):
        ...
        def my_prensor_op(original_prensor):
            ...
            return prensor.create_prensor_from_descendant_nodes(
                {
                    path.Path([]): original_prensor.node,
                    path.Path(["bar2"]): original_prensor.get_child_or_error(
                        "bar"
                    ).node,
                    path.Path(["keep_me2"]): original_prensor.get_child_or_error(
                        "keep_me"
                    ).node,
                }
            )
        ...
        my_output_schema = map_prensor_to_prensor.create_schema(
            ...
            children={
                ...
                "keep_me2": {"is_repeated": False, "dtype": tf.bool},
            },
        )
        ...
        result = map_prensor_to_prensor.map_prensor_to_prensor(
            ...
            paths_needed=[path.Path(["bar"]), path.Path(["keep_me"])],
