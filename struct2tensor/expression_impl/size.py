...
def size(
    root: expression.Expression, source_path: path.Path, new_field_name: path.Step
...
class SizeExpression(expression.Leaf):
    ...
    def calculate(
        ...
        if not isinstance(
            origin_value, (prensor.LeafNodeTensor, prensor.ChildNodeTensor)
        ):
            raise ValueError(
        ...
        if not isinstance(
            origin_parent_value, (prensor.ChildNodeTensor, prensor.RootNodeTensor)
        ):
            raise ValueError(
                "origin_parent_value must be a ChildNodeTensor "
                "or a RootNodeTensor, but was a " + str(type(origin_parent_value))
            )
