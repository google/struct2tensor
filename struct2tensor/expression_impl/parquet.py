...
def create_expression_from_parquet_file(
    filenames: List[str],
) -> placeholder._PlaceholderRootExpression:  # pylint: disable=protected-access
...
def calculate_parquet_values(
    ...
    Returns:
      A parquet dataset.
    """
    pqds = _ParquetDatasetWithExpression(
        expressions, root_exp, filenames, batch_size, options
    )
...
class _RawParquetDataset(tf.compat.v1.data.Dataset):
    ...
    def __init__(
        ...
        value_dtypes: List[tf.DType],
        parent_index_paths: List[str],
        path_index: List[int],
        batch_size: int,
    ):
...
class ParquetDataset(_RawParquetDataset):
    ...
    def __init__(self, filenames: List[str], value_paths: List[str], batch_size: int):
        """Creates a ParquetDataset.
    ...
    def _get_column_dtypes(
        self, metadata_file: str, value_paths: List[str]
    ) -> List[Union[tf.DType, None]]:
    ...
    def _create_children_spec(
        self, field: pa.lib.Field, index_and_paths: List[Tuple[int, List[path.Step]]]
    ...
    def _create_prensor_spec(self) -> prensor._PrensorTypeSpec:  # pylint: disable=protected-access
        ...
        return prensor._PrensorTypeSpec(
            None,
            prensor._PrensorTypeSpec._NodeType.ROOT,
            tf.int64,
            [
    ...
    def _create_parent_index_paths_and_index_from_type_spec(
        ...
        for field_tuple in fields:
            ...
            level += 1
            self._create_parent_index_paths_and_index_from_type_spec(spec, index, level)
            level -= 1
            index += 1
