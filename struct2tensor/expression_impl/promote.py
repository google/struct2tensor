...
class PromoteExpression(expression.Leaf):
    ...
    def __init__(
        self, origin: expression.Expression, origin_parent: expression.Expression
    ):
        super().__init__(
    ...
    def calculate(
        ...
        new_parent_index = tf.gather(
            origin_parent_value.parent_index, origin_value.parent_index
        )
        return prensor.LeafNodeTensor(
            new_parent_index, origin_value.values, self.is_repeated
        )
...
def _feature_is_dense(feature: schema_pb2.Feature) -> bool:
    return (
        ...
        and feature.value_count.HasField("min")
        and feature.value_count.HasField("max")
...
def _get_promote_schema_feature(
    original: Optional[schema_pb2.Feature], parent: Optional[schema_pb2.Feature]
    ...
    result.lifecycle_stage = _min_lifecycle_stage(
        original.lifecycle_stage, parent.lifecycle_stage
    )
...
def _promote_impl(
    root: expression.Expression, p: path.Path, new_field_name: path.Step
    ...
    return expression_add.add_paths(
        root,
        {
            new_path: promote_expression_factory(
