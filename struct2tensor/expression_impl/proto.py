...
def create_transformed_field(
    ...
    if isinstance(source_expr, _TransformProtoChildExpression):
        ...
        # from it.
        def final_transform(
            parent_indices: tf.Tensor, values: tf.Tensor
    ...
    transformed_expr = _TransformProtoChildExpression(
        ...
        backing_str_tensor=source_expr._backing_str_tensor,
...
class _ProtoRootNodeTensor(prensor.RootNodeTensor):
    ...
    def __init__(
        self, size: tf.Tensor, fields: Mapping[StrStep, struct2tensor_ops._ParsedField]
    ):
        super().__init__(size)
        self.fields = fields
...
class _AbstractProtoChildExpression(expression.Expression):
    ...
    def calculate(
        ...
        if isinstance(parent_value, (_ProtoRootNodeTensor, _ProtoChildNodeTensor)):
            ...
            return self.calculate_from_parsed_field(parsed_field, destinations, options)
...
class _ProtoLeafExpression(_AbstractProtoChildExpression):
    ...
    def calculate_from_parsed_field(
        ...
        return prensor.LeafNodeTensor(
            parsed_field.index, parsed_field.value, self.is_repeated
        )

    def calculation_equal(self, expr: expression.Expression) -> bool:
        # pylint: disable=protected-access
        return (
            isinstance(expr, _ProtoLeafExpression)
            and self._field_descriptor == expr._field_descriptor
            and self.name_as_field == expr.name_as_field
        )
...
class _ProtoChildExpression(_AbstractProtoChildExpression):
    ...
    def __init__(
        ...
        """
        super().__init__(parent, name_as_field, is_repeated, None, backing_str_tensor)
        self._desc = desc
    ...
    def calculate_from_parsed_field(
        ...
        fields = parse_message_level_ex.parse_message_level_ex(
            ...
            honor_proto3_optional_semantics=options.experimental_honor_proto3_optional_semantics,
        )
    ...
    def calculation_equal(self, expr: expression.Expression) -> bool:
        ...
        return (
            self._desc == expr._desc  # pylint: disable=protected-access
            and self.name_as_field == expr.name_as_field
        )
...
class _TransformProtoChildExpression(_ProtoChildExpression):
    ...
    def __init__(
        ...
        super().__init__(parent, desc, is_repeated, name_as_field, backing_str_tensor)
    ...
    def calculate_from_parsed_field(
        ...
        return _ProtoChildNodeTensor(
            transformed_parent_indices, self.is_repeated, fields
        )

    def calculation_equal(self, expr: expression.Expression) -> bool:
        return (
            isinstance(expr, _TransformProtoChildExpression)
            and self._desc == expr._desc  # pylint: disable=protected-access
            and self.name_as_field == expr.name_as_field
...
class _ProtoRootExpression(expression.Expression):
    ...
    def _get_child_impl(self, field_name: path.Step) -> Optional[expression.Expression]:
        return _get_child(self, self._descriptor, field_name, self._tensor_of_protos)
...
ProtoExpression = Union[
    ...
    _ProtoChildExpression,  # pylint: disable=invalid-name
...
def _get_any_child(
    ...
    if path.is_extension(field_name):
        full_name_child = parse_message_level_ex.get_full_name_from_any_step(field_name)
        if full_name_child is None:
            return None
        field_message = desc.file.pool.FindMessageTypeByName(full_name_child)
        return _ProtoChildExpression(
            parent, field_message, False, field_name, backing_str_tensor
        )
    return _get_child_helper(
        parent, desc.fields_by_name.get(field_name), field_name, backing_str_tensor
    )


def _is_map_field_desc(field_desc: descriptor.FieldDescriptor) -> bool:
    return field_desc.message_type and field_desc.message_type.GetOptions().map_entry
...
def _get_map_child(
    ...
    return _get_child_helper(parent, value_field_desc, field_name, backing_str_tensor)
...
def _get_child(
    ...
    return _get_child_helper(
        parent, _get_field_descriptor(desc, field_name), field_name, backing_str_tensor
    )


def _get_needed_fields(destinations: Sequence[expression.Expression]) -> Set[StrStep]:
