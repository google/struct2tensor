...
class Schema:
    ...
    def __str__(self) -> str:
        return (
            f"Schema(is_repeated:{self._is_repeated} type:{self._type}"
            " "
...
def create_schema(
    ...
    child_schemas = {k: _create_schema_helper(v) for k, v in children_dict.items()}
    return Schema(
...
class _PrensorAsRootNodeTensor(prensor.RootNodeTensor):
    ...
    def __init__(self, prensor_tree: prensor.Prensor, root: prensor.RootNodeTensor):
...
class _PrensorAsChildNodeTensor(prensor.ChildNodeTensor):
    ...
    def __init__(self, prensor_tree: prensor.Prensor, child: prensor.ChildNodeTensor):
        """Call _tree_as_node instead."""
...
class _PrensorAsLeafNodeTensor(prensor.LeafNodeTensor):
    ...
    def __init__(self, prensor_tree: prensor.Prensor, leaf: prensor.LeafNodeTensor):
        """Call _tree_as_node instead."""
...
_TreeAsNode = Union[
    _PrensorAsLeafNodeTensor, _PrensorAsChildNodeTensor, _PrensorAsRootNodeTensor
]


def _get_schema_or_error(parent: expression.Expression, step: path.Step) -> Schema:
...
class _PrensorOpChildExpression(expression.Expression):
    ...
    def __init__(self, parent: expression.Expression, step: path.Step, schema: Schema):
        super().__init__(
            schema.is_repeated,
            schema.type,
    ...
    def calculate(
        ...
        if not isinstance(
            parent_result,
            (
                _PrensorAsLeafNodeTensor,
                _PrensorAsChildNodeTensor,
