...
def map_values_anonymous(
    ...
    return map_many_values(
        root,
        source_path.get_parent(),
        [source_path.field_list[-1]],
        operation,
        dtype,
...
class _MapValuesExpression(expression.Expression):
    ...
    def __init__(
        self,
        origin: Sequence[expression.Expression],
        operation: Callable[..., tf.Tensor],
        dtype: tf.DType,
    ):
