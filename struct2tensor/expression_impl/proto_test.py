...
def _get_user_info_with_extension():
    ...
    my_user_info.Extensions[test_extension_pb2.MyExternalExtension.ext].special = "shhh"
...
class ProtoTest(absltest.TestCase):
    ...
    def test_create_expression_from_proto_with_root(self):
        ...
        self.assertEqual(
            expr.known_field_names(), frozenset({"event", "session_id", "session_info"})
        )
    ...
    def test_create_expression_from_proto_with_any_type_url(self):
        ...
        expr = _get_expression_with_any()
        any_expr = expr.get_child_or_error("my_any")
        simple_expr = expr.get_descendant_or_error(path.Path(["my_any", "type_url"]))
        self.assertFalse(simple_expr.is_repeated)
    ...
    def test_create_reversed_field_nested(self):
        expr = proto_test_util._get_expression_from_session_empty_user_info()
        first_reverse = proto.create_transformed_field(
            expr, path.Path(["event"]), "reversed_event", _reverse_values
        )
        second_reverse = proto.create_transformed_field(
            first_reverse,
            path.Path(["reversed_event", "action"]),
            "reversed_action",
            _reverse_values,
        )
...
class ProtoValuesTest(tf.test.TestCase, parameterized.TestCase):
    ...
    @parameterized.named_parameters(("string_view", True), ("no_string_view", False))
    ...
    def test_create_expression_from_proto_with_any(self, use_string_view):
        ...
        child_node = expression_test_util.calculate_value_slowly(
            simple_expr, options=self._get_calculate_options(use_string_view)
        ).parent_index
    ...
    def test_project_proto_map(self, use_string_view):
        ...
        self.assertAllEqual(feature1, [[[[[b"hello", b"world"]]]], [[[[b"deadbeef"]]]]])
    ...
    def test_project_proto_map_leaf_value(self, use_string_view):
        ...
        expr = proto_test_util.text_to_expression(protos, test_map_pb2.MessageWithMap)
        result = expression_test_util.calculate_list_map(
            expr.project(
                [
    ...
    @parameterized.named_parameters(("string_view", True), ("no_string_view", False))
    def test_transformed_field_values_with_transformed_parent(self, use_string_view):
        expr = proto_test_util._get_expression_from_session_empty_user_info()
        ...
        self.assertAllEqual(
            result["reversed_event.reversed_action.doc_id"],
            [
                [[[b"b"], [b"a"], []], [[b"c"]], [[b"f"], [b"e"]]],
                [[[b"g"], [b"j"]], [[b"i"], [b"h"]]],
            ],
    ...
    @parameterized.named_parameters(("string_view", True), ("no_string_view", False))
    def test_transformed_field_values_with_multiple_transforms(self, use_string_view):
        expr = proto_test_util._get_expression_from_session_empty_user_info()
        reversed_events_expr = proto.create_transformed_field(
            expr, path.Path(["event"]), "reversed_event", _reverse_values
        )
        ...
        self.assertAllEqual(
            result["reversed_reversed_event.action.doc_id"],
            [
                [[[b"a"], [b"b"]], [[b"c"], []], [[b"e"], [b"f"]]],
                [[[b"g"]], [[b"h"], [b"i"], [b"j"]]],
            ],
