    ...
    def _assertPrensorEqual(self, result, expected):
        ...
        if result.is_leaf:
            ...
            for child_step in res_children:
                self._assertPrensorEqual(
                    res_children[child_step], exp_children[child_step]
                )
...
class ParquetDatasetOutputsPrensorTest(ParquetDatasetTestBase):
    ...
    def testMultipleColumnsTwoRowGroupsAndEqualBatchSize_OutputsPrensor(self):
        ...
        expected_prensor = prensor.create_prensor_from_descendant_nodes(
            {
                path.Path([]): prensor.RootNodeTensor(tf.constant(2, dtype=tf.int64)),
                path.Path(["DocId"]): prensor.LeafNodeTensor(
                    tf.constant([0, 1], dtype=tf.int64),
                    tf.constant([10, 20], dtype=tf.int64),
                    True,
                ),
                path.Path(["Name"]): prensor.ChildNodeTensor(
                    tf.constant([0, 0, 0, 1], dtype=tf.int64), True
                ),
                path.Path(["Name", "Language"]): prensor.ChildNodeTensor(
                    tf.constant([0, 0, 2], dtype=tf.int64), True
                ),
                path.Path(["Name", "Language", "Code"]): prensor.LeafNodeTensor(
                    tf.constant([0, 1, 2], dtype=tf.int64),
                    tf.constant([b"en-us", b"en", b"en-gb"]),
                    True,
                ),
                path.Path(["Name", "Language", "Country"]): prensor.LeafNodeTensor(
                    tf.constant([0, 2], dtype=tf.int64),
                    tf.constant([b"us", b"gb"]),
                    True,
                ),
            }
        )
...
class ParquetDatasetWithExpressionTest(ParquetDatasetTestBase):
    ...
    def testPromoteAndProjectExpression(self):
        filenames = ["struct2tensor/testdata/parquet_testdata/dremel_example.parquet"]
        batch_size = 2
        exp = parquet.create_expression_from_parquet_file(filenames)
        new_exp = promote.promote(
            exp, path.Path(["Name", "Language", "Code"]), "new_code"
        )
        ...
        new_code_expected = prensor.create_prensor_from_descendant_nodes(
            {
                ...
                path.Path(["Name", "new_code"]): prensor.LeafNodeTensor(
