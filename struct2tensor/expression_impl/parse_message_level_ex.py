...
def parse_message_level_ex(
    ...
    map_fields = _get_map_parsed_fields(
        desc, regular_field_map, field_names, backing_str_tensor
    )
...
def get_full_name_from_any_step(step: ProtoFieldName) -> Optional[ProtoFieldName]:
...
def _any_indices_with_type(
    type_url: struct2tensor_ops._ParsedField, full_name: ProtoFullName
) -> tf.Tensor:
    ...
    second_column = tf.reshape(
        tf.sparse.to_dense(
            tf.sparse.slice(
                tensors_parsed, tf.constant([0, 1], dtype=tf.int64), second_column_shape
            ),
            default_value="",
        ),
        [-1],
...
def _get_any_parsed_field(
    value_field: struct2tensor_ops._ParsedField,
    ...
    [index_to_solution_index, index_to_values] = struct2tensor_ops.equi_join_indices(
        indices_with_type, value_field.index
    )
...
def _get_any_parsed_fields(
    ...
    for x in field_names:
        if path.is_extension(x):
            result.append(
                _get_any_parsed_field(
                    raw_parsed_fields["value"], raw_parsed_fields["type_url"], x
                )
            )
...
def _get_field_names_to_parse(
    desc: descriptor.Descriptor, needed_field_names: Set[StrStep]
) -> Sequence[ProtoFieldName]:
...
def _get_map_parsed_fields(
    ...
    for map_field_name, v in maps_to_parse.items():
        ...
        for map_key, [value, parent_index] in zip(
            keys_needed, values_and_parent_indices
        ):
...
def _get_parsed_field(
    data: Sequence[struct2tensor_ops._ParsedField], field_name: ProtoFieldName
