...
def get_positional_index(
    expr: expression.Expression, source_path: path.Path, new_field_name: path.Step
    ...
    return expression_add.add_paths(
        expr,
        {
            new_path: _PositionalIndexExpression(
...
def get_index_from_end(
    t: expression.Expression, source_path: path.Path, new_field_name: path.Step
) -> Tuple[expression.Expression, path.Path]:
    ...
    work_expr = expression_add.add_paths(
        work_expr,
        {
            new_path: _PositionalIndexFromEndExpression(
    ...
    def calculate(
        ...
        if isinstance(origin, (prensor.LeafNodeTensor, prensor.ChildNodeTensor)):
            return prensor.LeafNodeTensor(
                origin.parent_index, origin.get_positional_index(), self.is_repeated
            )
...
class _PositionalIndexFromEndExpression(expression.Leaf):
    ...
    def __init__(
        self, positional_index: expression.Expression, size_inp: expression.Expression
    ):
