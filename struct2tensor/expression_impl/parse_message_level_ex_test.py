...
class ParseMessageLevelExTest(parameterized.TestCase, tf.test.TestCase):
    ...
    def test_any_field(self):
        ...
        result_optional_int32 = [
            _get_optional_int32(x) for x in self.evaluate(result[_ALLSIMPLE][_VALUE])
        ]
    ...
    def test_any_field_special_and_type_url(self):
        result = _run_parse_message_level_ex(
            _create_any_protos(), {_TYPE_URL, _ALLSIMPLE}
        )
        self.assertIn(_TYPE_URL, result)
        self.assertAllEqual(result[_TYPE_URL][_INDEX], [0, 1, 2])
    ...
    def test_full_name_from_any_step(self):
        ...
        self.assertIsNone(parse_message_level_ex.get_full_name_from_any_step("broken"))
        self.assertIsNone(parse_message_level_ex.get_full_name_from_any_step("(broken"))
        self.assertIsNone(parse_message_level_ex.get_full_name_from_any_step("broken)"))
