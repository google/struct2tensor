...
def _get_mask(
    ...
    work_expr, mask_for_negative_threshold = map_values.map_values_anonymous(
        work_expr,
        index_from_end,
        lambda x: relation(x, tf.cast(threshold, tf.int64)),
        tf.bool,
    )
...
def _get_begin_mask(
    expr: expression.Expression, p: path.Path, begin: IndexValue
) -> Tuple[expression.Expression, path.Path]:
...
def _get_end_mask(
    t: expression.Expression, p: path.Path, end: IndexValue
) -> Tuple[expression.Expression, path.Path]:
...
def _get_slice_mask(
    ...
    return map_values.map_many_values(
        work_expr,
        p.get_parent(),
        [x.field_list[-1] for x in [begin_mask, end_mask]],
        tf.logical_and,
        tf.bool,
        path.get_anonymous_field(),
    )
