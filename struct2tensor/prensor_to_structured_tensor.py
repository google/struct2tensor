...
def _prensor_to_field_map(
    p_fields: Mapping[path.Step, prensor.Prensor], nrows: tf.Tensor
) -> Mapping[path.Step, structured_tensor.StructuredTensor]:
...
def _leaf_node_to_ragged_tensor(
    node: prensor.LeafNodeTensor, nrows: tf.Tensor
) -> tf.RaggedTensor:
