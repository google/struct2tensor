...
def structured_tensor_to_prensor(
    st: structured_tensor.StructuredTensor, default_field_name: path.Step = "data"
) -> prensor.Prensor:
    ...
    if len(row_partitions) >= 1:
        ...
        return prensor.create_prensor_from_root_and_children(
            prensor.RootNodeTensor((st).nrows()), {default_field_name: child_prensor}
        )
    if st.rank == 1:
        return prensor.create_prensor_from_root_and_children(
...
def _structured_tensor_prensor_map(
    st: structured_tensor.StructuredTensor, default_field_name: path.Step
) -> Mapping[path.Step, prensor.Prensor]:
    """Creates a map of fields, to put in a child or root prensor."""
    return {
        k: _structured_tensor_field_to_prensor(st.field_value(k), default_field_name)
...
def _expand_dims_scalar(st: structured_tensor.StructuredTensor):
    ...
    return structured_tensor.StructuredTensor.from_fields(new_fields, shape=new_shape)
...
def _structured_tensor_field_to_prensor(
    field_value: Union[structured_tensor.StructuredTensor, tf.RaggedTensor, tf.Tensor],
    default_field_name: path.Step,
) -> prensor.Prensor:
...
def _row_partition_to_child_node_tensor(row_partition: RowPartition):
    ...
    return prensor.ChildNodeTensor(
        tf.cast(row_partition.value_rowids(), tf.int64), is_repeated=True
    )


def _one_child_prensor(
...
def _structured_tensor_to_child_prensor(
    ...
    return _structured_tensor_to_child_prensor(_expand_dims(st, 1), default_field_name)


def _to_leaf_prensor_helper(
    rt: tf.RaggedTensor, default_field_name: path.Step
) -> prensor.Prensor:
...
def _fully_partitioned_ragged_tensor(
    rt: Union[tf.RaggedTensor, tf.Tensor], dtype=tf.dtypes.int64
):
...
def _to_leaf_prensor(
    rt: Union[tf.RaggedTensor, tf.Tensor], default_field_name: path.Step
) -> prensor.Prensor:
