...
class PrensorTest(tf.test.TestCase):
    ...
    def test_prensor_children_ordered(self):
        def _recursively_check_sorted(p):
            self.assertEqual(
                list(p.get_children().keys()), sorted(p.get_children().keys())
            )
    ...
    def test_prensor_to_ragged_tensors(self):
        for options in _OPTIONS_TO_TEST:
            ...
            self.assertAllEqual(
                string_np_map["doc.bar"].to_list(),
                [[[b"a"]], [[b"b", b"c"], [b"d"]], []],
            )

            self.assertAllEqual(
                string_np_map["doc.keep_me"].to_list(), [[[False]], [[True], []], []]
            )
            self.assertAllEqual(
                string_np_map["user.friends"].to_list(),
                [[[b"a"]], [[b"b", b"c"], [b"d"]], [[b"e"]]],
            )
    ...
    def test_prensor_to_ragged_tensor(self):
        for options in _OPTIONS_TO_TEST:
            pren = prensor_test_util.create_nested_prensor()
            ragged_tensor = pren.get_ragged_tensor(path.create_path("doc.bar"), options)
    ...
    def test_prensor_to_sparse_tensors(self):
        for options in _OPTIONS_TO_TEST:
            ...
            self.assertAllEqual(
                string_tensor_map["doc.bar"].indices,
                [[0, 0, 0], [1, 0, 0], [1, 0, 1], [1, 1, 0]],
            )
            self.assertAllEqual(
                string_tensor_map["doc.bar"].values, [b"a", b"b", b"c", b"d"]
            )
            self.assertAllEqual(
                string_tensor_map["doc.keep_me"].indices, [[0, 0], [1, 0]]
            )
            self.assertAllEqual(string_tensor_map["doc.keep_me"].values, [False, True])
            ...
            self.assertAllEqual(
                string_tensor_map["user.friends"].values, [b"a", b"b", b"c", b"d", b"e"]
            )
    ...
    def test_is_leaf(self):
        ...
        self.assertFalse(expression.get_descendant_or_error(path.Path(["doc"])).is_leaf)
        self.assertFalse(expression.get_descendant_or_error(path.Path([])).is_leaf)
        self.assertTrue(
            expression.get_descendant_or_error(path.Path(["user", "friends"])).is_leaf
        )
        self.assertTrue(
            expression.get_descendant_or_error(path.Path(["doc", "keep_me"])).is_leaf
        )
    ...
    def test_get_sparse_tensors_simple(self):
        ...
        for options in _OPTIONS_TO_TEST:
            ...
            self.assertAllEqual(
                string_tensor_map["foorepeated"].indices,
                [[0, 0], [1, 0], [1, 1], [2, 0]],
            )
    ...
    def test_get_sparse_tensor(self):
        expression = prensor_test_util.create_simple_prensor()
        sparse_tensor = prensor._get_sparse_tensor(expression, path.create_path("foo"))
        self.assertAllEqual(sparse_tensor.indices, [[0], [1], [2]])
        self.assertAllEqual(sparse_tensor.dense_shape, [3])
        self.assertAllEqual(sparse_tensor.values, [9, 8, 7])
    ...
    def test_get_sparse_tensors_simple_dense(self):
        ...
        for options in _OPTIONS_TO_TEST:
            ...
            self.assertAllEqual(
                string_tensor_map["foorepeated"], [[9, 0], [8, 7], [6, 0]]
            )
