...
def _make_dict_runnable(level_as_dict):
  """Prepares output of parse_full_message_level_as_dict for sess.run()."""
  result = {}
...
class PrensorOpsTest(tf.test.TestCase):
  ...
  def test_parse_full_message_level_for_event(self):
    with self.session(use_gpu=False) as sess:
      ...
      indices_out, values_out = sess.run([doc_id.index, doc_id.value])
      serialized_actions = [
          proto.SerializeToString() for proto in [action0, action1, action2]
      ]

      self.assertAllEqual(indices_out, [0, 0, 1])
      self.assertAllEqual(values_out, serialized_actions)
  ...
  def test_parse_full_message_level_for_simple_action_multiple(self):
    ...
    with self.session(use_gpu=False) as sess:
      ...
      [values_out, indices_out] = sess.run([doc_id.value, doc_id.index])
      self.assertAllEqual(indices_out, [0, 0, 2, 2])
      self.assertAllEqual(values_out, [b"b", b"c", b"d", b"e"])
  ...
  def test_parse_full_message_level_for_all_simple_repeated_repeated(self):
    ...
    with self.session(use_gpu=False) as sess:
      ...
      result = sess.run(
          _get_full_message_level_runnable([
              all_simple,
              test_pb2.AllSimple(),
  ...
  def test_parse_full_message_level_for_all_simple_repeated(self):
    ...
    with self.session(use_gpu=False) as sess:
      ...
      all_simple.repeated_double.append(1.5)
      all_simple.repeated_double.append(2.5)
      result = sess.run(_get_full_message_level_runnable([all_simple]))
  ...
  def test_parse_full_message_level_action(self):
    with self.session(use_gpu=False) as sess:
      ...
      result = sess.run(_get_full_message_level_runnable([action]))
  ...
  def test_parse_message_level(self):
    with self.session(use_gpu=False) as sess:
      ...
      [values_out, indices_out] = sess.run([values, indices])
      self.assertAllEqual(indices_out, [0])
      self.assertAllEqual(values_out, [3])
  ...
  def test_parse_extension(self):
    with self.session(use_gpu=False) as sess:
      ...
      [values_out, indices_out] = sess.run([values, indices])
      self.assertAllEqual(indices_out, [0])
      self.assertAllEqual(values_out, [expected_value.SerializeToString()])
  ...
  def test_parse_packed_fields(self):
    with self.session(use_gpu=False) as sess:
      ...
      indices_out, values_out = sess.run((indices, values))

      for indices in indices_out.values():
        self.assertAllEqual(indices, [0, 0, 0, 1, 1, 1])
      for field_name, values in values_out.items():
        self.assertAllEqual(
            values,
  ...
  def test_make_repeated_basic(self):
    with self.session(use_gpu=False) as sess:
      ...
      sparse_tensor_out = sess.run(sparse_tensor)
      self.assertAllEqual(
          sparse_tensor_out.indices,
  ...
  def test_make_repeated_empty(self):
    with self.session(use_gpu=False) as sess:
      ...
      sparse_tensor_out = sess.run(sparse_tensor)
      self.assertAllEqual(sparse_tensor_out.indices.shape, [0, 2])
  ...
  def test_equi_join_indices(self):
    with self.session(use_gpu=False) as sess:
      ...
      [index_a_out, index_b_out] = sess.run([index_a, index_b])
      self.assertAllEqual(index_a_out, [0, 0, 1, 1, 4, 4, 5])
      self.assertAllEqual(index_b_out, [0, 1, 0, 1, 2, 3, 4])
  ...
  def test_equi_join_indices_2(self):
    with self.session(use_gpu=False) as sess:
      ...
      [index_a, index_b] = struct2tensor_ops.equi_join_indices(a, b)
      [index_a_out, index_b_out] = sess.run([index_a, index_b])
      self.assertAllEqual(index_a_out, [0, 1, 2, 3])
      self.assertAllEqual(index_b_out, [0, 1, 1, 2])
  ...
  def test_equi_join_indices_empty_a(self):
    with self.session(use_gpu=False) as sess:
      ...
      [index_a, index_b] = struct2tensor_ops.equi_join_indices(a, b)
      [index_a_out, index_b_out] = sess.run([index_a, index_b])
      self.assertAllEqual(index_a_out, [])
      self.assertAllEqual(index_b_out, [])
  ...
  def test_equi_join_indices_for_broadcast(self):
    ...
    with self.session(use_gpu=False) as sess:
      ...
      [index_a, index_b] = struct2tensor_ops.equi_join_indices(a, b)
      [index_a_out, index_b_out] = sess.run([index_a, index_b])
      self.assertAllEqual(index_a_out, [0, 1, 2])
      self.assertAllEqual(index_b_out, [0, 1, 1])
  ...
  def test_run_length_before(self):
    ...
    with self.session(use_gpu=False) as sess:
      ...
      b_out = sess.run(b)
      self.assertAllEqual(b_out, [0, 0, 1, 0, 0, 1, 0])
...
class DecodeProtoMapOpTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_signed_integer_key_types(self, key_type):
    ...
    with self.session() as sess:
      [(values_42, indices_42), (values_n42, indices_n42),
       (values_0, indices_0)] = sess.run(
           self._parse_map_entry([message_with_map], field_name,
                                 ["42", "-42", "0"]))
  ...
  def test_unsigned_integer_key_types(self, key_type):
    ...
    with self.session() as sess:
      [(values_42, indices_42), (values_0, indices_0)] = sess.run(
  ...
  def test_bool_key_type(self):
    ...
    with self.session() as sess:
      [(values_false, indices_false), (values_true, indices_true)] = sess.run(
  ...
  def test_signed_integer_value_types(self, value_type):
    ...
    with self.session() as sess:
      [(values_foo, indices_foo), (values_bar, indices_bar),
       (values_null, indices_null)] = sess.run(
           self._parse_map_entry([message_with_map], field_name,
                                 ["foo", "bar", ""]))
  ...
  def test_unsigned_integer_value_types(self, value_type):
    ...
    with self.session() as sess:
      ...
      [(values_foo, indices_foo), (values_null, indices_null)] = sess.run(
  ...
  def test_multiple_messages(self):
    ...
    with self.session() as sess:
      ...
      [(values_key1, indices_key1), (values_key2, indices_key2),
       (values_key3, indices_key3)] = sess.run(
           self._parse_map_entry(
