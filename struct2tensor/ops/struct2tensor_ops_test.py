...
class PrensorOpsTest(parameterized.TestCase, tf.test.TestCase):
            ...
            for num_fields_to_parse in range(len(all_fields_to_parse)):
                for comb in itertools.combinations(
                    ...
                    for f in parsed_fields:
                        self.assertAllEqual(
                            expected_field_value[f.field_name],
                            f.value,
    ...
    def test_out_of_order_repeated_fields_1(self):
        ...
        expected_field_value = {"query_token": [b"aaa", b"bbb"], "event_id": [b"abc"]}

        for fields_to_parse in [
            ["query_token"],
            ["event_id"],
    ...
    def test_out_of_order_repeated_fields_3(self):
        ...
        for fields_to_parse in [
            ["repeated_int64"],
            ["repeated_string"],
            ["repeated_string", "repeated_uint32", "repeated_int32"],
        ]:
            parsed_fields = struct2tensor_ops.parse_message_level(
    ...
    def test_parse_full_message_level_for_all_simple_repeated_repeated(self):
        ...
        self.assertAllEqual(
            result["repeated_int32"][VALUE], [32, 322, 32, 322, 32, 322]
        )
        self.assertAllEqual(result["repeated_uint32"][INDEX], [0, 0, 3, 3, 5, 5])
        self.assertAllEqual(
            result["repeated_uint32"][VALUE], [123, 1232, 123, 1232, 123, 1232]
        )
        ...
        self.assertAllEqual(
            result["repeated_uint64"][VALUE], [123, 1232, 123, 1232, 123, 1232]
        )
        self.assertAllEqual(result["repeated_float"][INDEX], [0, 0, 3, 3, 5, 5])
        self.assertAllEqual(
            result["repeated_float"][VALUE], [1.0, 2.0, 1.0, 2.0, 1.0, 2.0]
        )
        self.assertAllEqual(result["repeated_double"][INDEX], [0, 0, 3, 3, 5, 5])
        self.assertAllEqual(
            result["repeated_double"][VALUE], [1.5, 2.5, 1.5, 2.5, 1.5, 2.5]
        )
    ...
    def test_parse_message_level(self):
        ...
        [field_tuple] = struct2tensor_ops.parse_message_level(
            tensor_of_protos, test_pb2.Action().DESCRIPTOR, ["number_of_views"]
        )
    ...
    def test_parse_extension(self):
        ...
        user_info.Extensions[
            test_pb2.LocationOfExtension.special_user_info
        ].secret = "shhh"
    ...
    def test_parse_external_extension(self):
        user_info = test_pb2.UserInfo()
        user_info.Extensions[
            test_extension_pb2.MyExternalExtension.ext
        ].special = "shhh"
    ...
    def test_parse_packed_fields(self):
        ...
        parsed_tuples = struct2tensor_ops.parse_message_level(
            tensor_of_protos,
            test_pb2.HasPackedFields.DESCRIPTOR,
            [
                "packed_int32",
                "packed_uint32",
                "packed_int64",
        ...
        for field_name, value in values.items():
            self.assertAllEqual(
                value, list(getattr(message_with_packed_fields, field_name)) * 2
            )
...
_SIGNED_INTEGER_TYPES = ["int32", "int64", "sfixed32", "sfixed64", "sint32", "sint64"]
...
class DecodeProtoMapOpTest(parameterized.TestCase, tf.test.TestCase):
    def _parse_map_entry(self, messages_with_map, map_field_name, keys_needed):
        parsed_map_submessage = struct2tensor_ops.parse_message_level(
            ...
            test_map_pb2.MessageWithMap.DESCRIPTOR,
            [map_field_name],
        )[0]

        return struct2tensor_ops.parse_proto_map(
            parsed_map_submessage.value,
            parsed_map_submessage.index,
            parsed_map_submessage.field_descriptor.message_type,
            keys_needed,
        )
    ...
    def test_signed_integer_key_types(self, key_type):
        ...
        [(values_42, indices_42), (values_n42, indices_n42), (values_0, indices_0)] = (
            self._parse_map_entry([message_with_map], field_name, ["42", "-42", "0"])
        )
    ...
    def test_unsigned_integer_key_types(self, key_type):
        ...
        [(values_42, indices_42), (values_0, indices_0)] = self._parse_map_entry(
            [message_with_map], field_name, ["42", "0"]
        )
        self.assertAllEqual(values_42, [b"hello"])
        self.assertAllEqual(values_0, [])
        self.assertAllEqual(indices_42, [0])
        ...
        with self.assertRaisesRegex(
            ...
            self.evaluate(
                self._parse_map_entry(
                    [test_map_pb2.MessageWithMap()], "uint32_string_map", ["-42"]
                )
            )
    ...
    def test_invalid_int32_key(self):
        with self.assertRaisesRegex(
            ...
            self.evaluate(
                self._parse_map_entry(
                    [test_map_pb2.MessageWithMap()], "int32_string_map", ["foo"]
                )
            )
    ...
    def test_signed_integer_value_types(self, value_type):
        ...
        [
            ...
            (values_bar, indices_bar),
            (values_null, indices_null),
        ] = self._parse_map_entry([message_with_map], field_name, ["foo", "bar", ""])
    ...
    def test_enum_value_type(self):
        ...
        [(values_foo, indices_foo), (values_null, indices_null)] = (
            self._parse_map_entry([message_with_map], "string_enum_map", ["foo", ""])
        )
    ...
    def test_message_value_type(self):
        ...
        [(values_foo, indices_foo), (values_null, indices_null)] = (
            self._parse_map_entry([message_with_map], "string_message_map", ["foo", ""])
        )
    ...
    def test_multiple_messages(self):
        message_with_map1 = test_map_pb2.MessageWithMap(
            string_string_map={"key1": "foo", "key3": "bar"}
        )
        message_with_map2 = test_map_pb2.MessageWithMap()
        message_with_map3 = test_map_pb2.MessageWithMap(
            string_string_map={"key2": "baz", "key1": "kaz"}
        )
        [
            (values_key1, indices_key1),
            (values_key2, indices_key2),
            (values_key3, indices_key3),
        ] = self._parse_map_entry(
        ...
        with self.assertRaises(tf.errors.DataLossError):
            self.evaluate(
                struct2tensor_ops.parse_proto_map(
                    ...
                    test_map_pb2.MessageWithMap.DESCRIPTOR.fields_by_name[
                        "int32_string_map"
                    ].message_type,
                    ["0"],
                )
