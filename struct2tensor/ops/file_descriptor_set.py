...
def _are_dependencies_handled(
    file_descriptor: descriptor.FileDescriptor, dependencies: Set[descriptor.Descriptor]
) -> bool:
    """Returns True iff dependencies of descriptor are in dependencies."""
    return all(
        dependency in dependencies for dependency in file_descriptor.dependencies
    )


def _order_dependencies(
    file_descriptors: Set[descriptor.FileDescriptor],
    ...
    remaining_file_descriptors = sorted(list(file_descriptors), key=lambda x: x.name)
    ...
    if remaining_file_descriptors:
        ...
        raise ValueError(
            "Circular dependency "
            + str([file_desc.name for file_desc in remaining_file_descriptors])
        )
    return result
...
def _get_initial_file_descriptor_set(
    descriptor_type: descriptor.Descriptor, field_names: Sequence[str]
) -> Set[descriptor.FileDescriptor]:
