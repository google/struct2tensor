...
def _get_field_descriptor(descriptor_type: descriptor.Descriptor, field_name: str):
...
def parse_message_level(
    ...
    field_descriptors = [
        _get_field_descriptor(descriptor_type, field_name) for field_name in field_names
    ]
    output_types = [
...
def create_sparse_tensor_for_repeated(
    parent_index: tf.Tensor, values: tf.Tensor, dense_shape: tf.Tensor
) -> tf.SparseTensor:
...
def parse_proto_map(
    ...
    backing_str_tensor = (
        [backing_str_tensor] if tf.is_tensor(backing_str_tensor) else []
    )
