...
def calculate_value_slowly(
    ...
    """
    new_options = (
        calculate_options.get_default_options() if options is None else options
    )
...
class MockExpression(expression.Expression):
    ...
    def __init__(
        ...
        children: Optional[Mapping[path.Step, expression.Expression]] = None,
...
def get_mock_leaf(
    ...
    if calculate_is_identity:
        ...
        calculate_output = prensor.LeafNodeTensor(
            tf.constant([], dtype=tf.int64), tf.constant([], dtype=my_type), is_repeated
        )
