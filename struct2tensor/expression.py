...
promote = LazyLoader("promote", globals(), "struct2tensor.expression_impl.promote")

broadcast = LazyLoader(
    "broadcast", globals(), "struct2tensor.expression_impl.broadcast"
)

promote_and_broadcast = LazyLoader(
    "promote_and_broadcast",
    globals(),
    "struct2tensor.expression_impl.promote_and_broadcast",
)

map_values = LazyLoader(
    "map_values", globals(), "struct2tensor.expression_impl.map_values"
)
...
map_prensor = LazyLoader(
    "map_prensor", globals(), "struct2tensor.expression_impl.map_prensor"
)

apply_schema = LazyLoader(
    "apply_schema", globals(), "struct2tensor.expression_impl.apply_schema"
)

slice_expression = LazyLoader(
    "slice_expression", globals(), "struct2tensor.expression_impl.slice_expression"
)
...
class Expression(metaclass=abc.ABCMeta):
    ...
    def get_descendant_or_error(self, p: path.Path) -> "Expression":
        ...
        if result is None:
            raise ValueError(
                f"Missing path: {str(p)} in {self.schema_string(limit=20)}"
            )
        return result
    ...
    def get_known_descendants(self) -> Mapping[path.Path, "Expression"]:
        ...
        known_subexpressions = {
            k: v.get_known_descendants() for k, v in self.get_known_children().items()
        }
        result = {}
    ...
    def _schema_string_helper(
        self, field_name: path.Step, limit: Optional[int]
    ) -> List[str]:
        ...
        if self.type is None:
            ...
            result = [f"{repeated_as_string} {str(self.type)} {str(field_name)}"]
    ...
    def slice(
        ...
        """
        return slice_expression.slice_expression(
            self, path.create_path(source_path), new_field_name, begin, end
        )
    ...
    def broadcast(
        ...
        return broadcast.broadcast(
            self, path.create_path(source_path), sibling_field, new_field_name
        )
    ...
    def promote_and_broadcast(
        ...
        return promote_and_broadcast.promote_and_broadcast(
            self,
            {k: path.create_path(v) for k, v in path_dictionary.items()},
    ...
    def map_field_values(
        ...
        return map_values.map_values(
            self, path.create_path(source_path), operator, dtype, new_field_name
        )
    ...
    def apply(self, transform: Callable[["Expression"], "Expression"]) -> "Expression":
    ...
    def get_paths_with_schema(self) -> List[path.Path]:
        ...
        if not result:
            result.append(path.Path([], validate_step_format=self.validate_step_format))
        return result
