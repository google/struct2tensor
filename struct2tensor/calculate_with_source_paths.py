...
def calculate_prensors_with_source_paths(
    ...
    prensors, graph = calculate.calculate_prensors_with_graph(trees, options=options)
    proto_expressions = [
...
def requirements_to_metadata_proto(
    inp: Sequence[ProtoRequirements], result: query_metadata_pb2.QueryMetadata
) -> None:
    ...
    """
    for x in inp:
        _requirement_to_parsed_proto_info(
            x, result.parsed_proto_info.add()
    ...
    for expr in proto_expressions:
        ...
        def get_summary(tensor_of_protos, desc):
            for summary in result:
                if (
                    id(summary.tensor) == id(tensor_of_protos)
                    and summary.descriptor == desc
                ):
                    return summary
...
def _requirement_to_parsed_proto_info(
    inp: ProtoRequirements, result: query_metadata_pb2.ParsedProtoInfo
) -> None:
