...
def create_subtrees(
    ...
    subtrees = {}  # type:Mapping[path.Step, Mapping[path.Path, expression.Expression]]
...
def add_paths(
    root: expression.Expression, path_map: Mapping[path.Path, expression.Expression]
):
...
def _is_true_source_expression(
    candidate: expression.Expression, dest: expression.Expression
) -> bool:
...
def add_to(
    root: expression.Expression, origins: Mapping[path.Path, expression.Expression]
):
    ...
    path_map = {
        p: origin_root.get_descendant_or_error(p) for p, origin_root in origins.items()
    }
