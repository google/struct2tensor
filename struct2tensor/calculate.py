...
def calculate_values_with_graph(
    ...
    return (
        [expression_graph.get_value_or_die(x) for x in expressions],
        expression_graph,
    )
...
def calculate_values(
    ...
    return calculate_values_with_graph(
        expressions, options=options, feed_dict=feed_dict
    )[0]
...
def _get_prensor(
    subtree: Mapping[path.Path, expression.Expression],
...
class _ExpressionNode:
    ...
    def __init__(self, expr: expression.Expression):
        ...
        self.sources = [
            _get_earliest_equal_calculation(x) for x in expr.get_source_expressions()
        ]
    ...
    def __str__(self) -> str:
        return (
            f"expression: {str(self.expression)} sources: {str(self.sources)} destinations: "
    ...
    def calculate(
        ...
        if expected_type is None:
            if not isinstance(
                actual_value, (prensor.RootNodeTensor, prensor.ChildNodeTensor)
            ):
                raise self._create_value_error()
...
class ExpressionGraph:
    ...
    def get_value(self, expr: expression.Expression) -> Optional[prensor.NodeTensor]:
    ...
    def calculate_values(
        ...
        for node in self.ordered_node_list:
            ...
            side_info = (
                feed_dict[node.expression]
                if feed_dict and (node.expression in feed_dict)
                else None
            )
...
class OriginalExpressionGraph(ExpressionGraph):
    ...
    def _add_expressions(self, expressions: Sequence[expression.Expression]) -> None:
...
class CanonicalExpressionGraph(ExpressionGraph):
    ...
    def _create_node_if_not_exists(
