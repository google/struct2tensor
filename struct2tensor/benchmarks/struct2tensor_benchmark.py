    ...
    @parameterized.named_parameters(
        *[
            ...
            dict(
                ...
                fn_args=[
                    benchmark_pb2.FlatProto.DESCRIPTOR,
                    [s2t.path.Path(["int_values_1"]), s2t.path.Path(["int_values_2"])],
                ],
                proto_list_key="flat_protos",
            ),
            dict(
...
class PrensorToTensorBenchmarks(struct2tensor_benchmark_util.ProtoDataBenchmarks):
    ...
    @parameterized.named_parameters(
        *[
            dict(
                ...
                fn_args=[
                    benchmark_pb2.FlatProto.DESCRIPTOR,
                    [s2t.path.Path([f"int_values_{i + 1}"]) for i in range(n)],
                ],
                proto_list_key="flat_protos",
            )
            for n in [1, 2, 3, 4, 5]
        ...
        + [
            ...
            dict(
                ...
                fn_args=[
                    ...
                    [s2t.path.Path([f"int_values_{i}"]) for i in range(1, 101)],
                ],
    ...
    def test_to_dense(self, fn_name, fn_args, proto_list_key):
        ...
        self.run_benchmarks(
            fn_name, _get_prensor_to_dense_tensor_fn, fn_args, proto_list_key
        )
    ...
    def test_to_ragged(self, fn_name, fn_args, proto_list_key):
        ...
        self.run_benchmarks(
            fn_name, _get_prensor_to_ragged_tensor_fn, fn_args, proto_list_key
        )
    ...
    def test_to_sparse(self, fn_name, fn_args, proto_list_key):
        ...
        self.run_benchmarks(
            fn_name, _get_prensor_to_sparse_tensor_fn, fn_args, proto_list_key
        )
...
class TfExampleBenchmarks(struct2tensor_benchmark_util.ProtoDataBenchmarks):
        ...
        *[
        ...
        + [
            dict(
                ...
                fn_args=[
                    {"int_values_1": tf.io.FixedLenFeature(shape=[100], dtype=tf.int64)}
                ],
    ...
    def test_parse_tf_example(self, fn_name, fn_args, proto_list_key):
        self.run_benchmarks(fn_name, _get_parse_tf_example_fn, fn_args, proto_list_key)
    ...
    @parameterized.named_parameters(
        *[
            dict(
                ...
                fn_args=[{"int_values_1": tf.io.VarLenFeature(dtype=tf.int64)}],
                proto_list_key="tf_examples",
            )
            for n in [1, 2, 3, 4, 5]
        ]
    ...
    def test_parse_tf_example_to_sparse(self, fn_name, fn_args, proto_list_key):
        self.run_benchmarks(
            fn_name, _get_parse_tf_example_to_sparse_fn, fn_args, proto_list_key
        )
    ...
    @parameterized.named_parameters(
        *[
            dict(
                ...
                fn_args=[
                    {
                        "int_values_1": tf.io.RaggedFeature(
                            value_key="int_values_1", dtype=tf.int64
                        )
                    }
                ],
                proto_list_key="tf_examples",
...
def _get_promote_fn(session, proto_descriptor, path_to_promote):
    ...
    protos = tf.compat.v1.placeholder(dtype=tf.string, shape=(None,))
    expr = (
        s2t.expression_impl.proto.create_expression_from_proto(protos, proto_descriptor)
        .promote(path_to_promote, "new_child")
        .project([path_to_promote.prefix(-2).concat(s2t.path.Path(["new_child"]))])
    )
    [prensor] = s2t.calculate.calculate_prensors(
        [expr], options=s2t.calculate_options.get_options_with_minimal_checks()
    )
...
def _get_broadcast_fn(session, proto_descriptor, path_to_broadcast, sibling):
    ...
    expr = (
        s2t.expression_impl.proto.create_expression_from_proto(protos, proto_descriptor)
        .broadcast(path_to_broadcast, sibling, "new_child")
        .project(
            [
