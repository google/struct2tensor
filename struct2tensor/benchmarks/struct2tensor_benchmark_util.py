...
flags.DEFINE_bool(
    "test_mode", False, "if True, run all benchmarks with two iterations."
)
...
class Struct2tensorBenchmarksBase(parameterized.TestCase):
    ...
    def run_benchmarks(self, fn_name, get_benchmark_fn, fn_args, data_key):
        ...
        """
        print(
            f"BEGIN {fn_name}:\tNum Iterations\tTotal (wall) Time (s)\t"
        ...
        if FLAGS.test_mode:
            print(
                "WARNING: --test_mode is True. Setting iterations to 2 with sample "
                "size of 1."
            )
...
class ProtoDataBenchmarks(Struct2tensorBenchmarksBase):
    ...
    def setUpClass(cls):
        ...
        ds = tf.data.TFRecordDataset(
            os.path.join(_BASE_DIR, "deep_all_types_4096_positive.tfrecord.gz"), "GZIP"
        ...
        ds = tf.data.TFRecordDataset(
            os.path.join(_BASE_DIR, "flat_all_types_4096_positive.tfrecord.gz"), "GZIP"
        ).shuffle(shuffle_size)
        cls._flat_protos = [
            [list(ds.take(batch_size).as_numpy_iterator())]
            for batch_size in batch_sizes
        ...
        ds = tf.data.TFRecordDataset(
            os.path.join(_BASE_DIR, "flat_all_types_100_int_values_4096.tfrecord.gz"),
            "GZIP",
        ).shuffle(shuffle_size)
        ...
        ds = tf.data.TFRecordDataset(
            os.path.join(_BASE_DIR, "flat_100_int_features_4096.tfrecord.gz"), "GZIP"
        ).shuffle(shuffle_size)
        ...
        ds = tf.data.TFRecordDataset(
            os.path.join(
                _BASE_DIR, "flat_100_int_features_100_values_4096.tfrecord.gz"
            ),
            "GZIP",
        ).shuffle(shuffle_size)
        ...
        ds = tf.data.TFRecordDataset(
            os.path.join(_BASE_DIR, "tf_example_all_types_4096.tfrecord.gz"), "GZIP"
        ).shuffle(shuffle_size)
        cls._tf_examples = [
            [list(ds.take(batch_size).as_numpy_iterator())]
            for batch_size in batch_sizes
        ...
        ds = tf.data.TFRecordDataset(
            os.path.join(
                _BASE_DIR, "tf_example_1_int_feature_100_values_4096.tfrecord.gz"
            ),
            "GZIP",
        ).shuffle(shuffle_size)
        ...
        ds = tf.data.TFRecordDataset(
            os.path.join(
                _BASE_DIR, "tf_example_100_int_features_100_values_4096.tfrecord.gz"
            ),
            "GZIP",
        ).shuffle(shuffle_size)
        ...
        cls._data = {
            "deep_protos": cls._deep_protos,
            "flat_protos": cls._flat_protos,
            "flat_protos_1_feature_100_values": cls._flat_protos_1_feature_100_values,
            "flat_protos_100_features": cls._flat_protos_100_features,
            "flat_protos_100_features_100_values": cls._flat_protos_100_features_100_values,
            "tf_examples": cls._tf_examples,
            "tf_examples_1_feature_100_values": cls._tf_examples_1_feature_100_values,
            "tf_examples_100_features": cls._tf_examples_100_features,
            "tf_examples_100_features_100_values": cls._tf_examples_100_features_100_values,
        }
...
class OpsBenchmarks(Struct2tensorBenchmarksBase):
    ...
    def setUpClass(cls):
        ...
        cls._data = {"monotonic_increasing": [[a, b]], "random": [[rand_a, rand_b]]}
