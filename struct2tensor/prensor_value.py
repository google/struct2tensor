...
class LeafNodeValue:
    ...
    def __init__(self, parent_index: np.ndarray, values: np.ndarray, is_repeated: bool):
        """Creates a leaf node.
...
def _prensor_value_from_type_spec_and_component_values(
    ...
    component_values: Iterator[Union[int, np.ndarray]],
) -> PrensorValue:
    ...
    if prensor_type_spec._node_type == prensor_type_spec._NodeType.ROOT:
        ...
        node = ChildNodeValue(next(component_values), prensor_type_spec._is_repeated)
    else:
